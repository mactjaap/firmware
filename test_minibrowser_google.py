...
def main():
    ...
    try:
        ...
        def test_google_search():
            ...
            # Activate Google search field [3].
            badge.clear_log()
            badge.type_text("3")
            badge.settle(0.2)
            badge.enter()
            ...
            # Enter the actual search query.
            badge.type_text("esp32")
            badge.settle(0.3)
            badge.enter()
            badge.settle(0.4)

            # Activate Google Search button [4].
            badge.type_text("4")
            badge.settle(0.2)
            badge.enter()
            badge.settle(0.5)
            ...
            # Give the parser/renderer time to emit the complete CONTENT block.
            badge.settle(2.0)

            content = latest_content_lines(
                badge,
                max_lines=8
            )
            ...
            if not content:
                raise RuntimeError(
                    "Google returned HTTP 200 but no parsed CONTENT block was captured"
                )

            # Prove the returned page contains the query or ESP32-related text.
            if not any(
                "esp32" in line.lower()
                for line in content
            ):
                # Search the whole captured page, not only the first displayed lines.
                ...
                if not any(
                    ...
                    raise RuntimeError(
                        "Google returned HTTP 200, but no ESP32 text was found in the parsed result"
                    )
